-- The admin-only key diagnostic may run before public chat is enabled, but
-- still consumes the same atomic daily model allowance.
...
begin
  ...
  if model_count >= cfg.daily_model_limit then return false; end if;
