...
import { Input } from '@/components/ui/input'; // IMPORTANTE: Importado para o campo de pesquisa
...
import { Building2, Mail, User, MapPin, CheckCircle, Award, Calendar, Trash2, Clock, XCircle, Camera, CameraOff, Search, Layers } from 'lucide-react';
...
export const EmpresasSection = () => {
  ...
  const [filtroCategoria, setFiltroCategoria] = useState<string>('todas'); // NOVO: Estado para filtro de categoria
  const [buscaNome, setBuscaNome] = useState<string>(''); // NOVO: Estado para campo de busca por nome
  ...
  // NOVO: Descobre todas as categorias únicas que existem nas empresas retornadas para listar no Select
  const categoriasUnicas = Array.from(
    new Set(
      empresas
        .map((e) => e.categorias?.nome)
        .filter((nome): nome is string => Boolean(nome))
    )
  ).sort();

  // MODIFICADO: Lógica de filtros aprimorada para incluir busca por nome e filtro de categoria
  const empresasFiltradas = empresas.filter((empresa) => {
    // Filtro por Status
    const bateStatus = filtroStatus === 'todos' || empresa.status_aprovacao === filtroStatus;
    
    // Filtro por Categoria
    const bateCategoria = filtroCategoria === 'todas' || empresa.categorias?.nome === filtroCategoria;
    
    // Filtro por Busca de Texto (Nome da empresa, nome do usuário ou e-mail)
    const termo = buscaNome.toLowerCase().trim();
    const bateBusca = 
      termo === '' ||
      empresa.nome.toLowerCase().includes(termo) ||
      (empresa.usuario?.nome && empresa.usuario.nome.toLowerCase().includes(termo)) ||
      (empresa.usuario?.email && empresa.usuario.email.toLowerCase().includes(termo));

    return bateStatus && bateCategoria && bateBusca;
  });
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {/* MODIFICADO: Nova área de filtros contendo barra de pesquisa e seletores lado a lado */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end bg-card p-4 rounded-xl border">
        {/* Campo de Pesquisa por texto */}
        <div className="space-y-2">
          <span className="text-sm font-medium flex items-center gap-1">
            <Search className="w-4 h-4 text-muted-foreground" /> Buscar empresa:
          </span>
          <Input
        ...
        <div className="space-y-2">
          <span className="text-sm font-medium flex items-center gap-1">
            <Layers className="w-4 h-4 text-muted-foreground" /> Filtrar por categoria:
          </span>
        ...
        {empresasFiltradas.length === 0 && (
          <NeonCard>
            <CardContent className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                {buscaNome || filtroStatus !== 'todos' || filtroCategoria !== 'todas'
