      ...
      <div className="relative overflow-hidden rounded-3xl border border-white/10 p-5 shadow-xl sm:p-7" style={{ background: `linear-gradient(135deg, ${program.cor_principal}, #171022 72%)` }}>
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="max-w-2xl text-white">
            ...
            <p className="mt-2 text-sm text-white/75">{program.descricao || `Junte ${program.carimbos_necessarios} ${program.rotulo_carimbo}(s) e ganhe: ${program.recompensa_descricao}.`}</p>
            {card && (
              <div className="mt-4 max-w-md">
                <div className="mb-1 flex justify-between text-xs"><span>{card.saldo} de {program.carimbos_necessarios}</span><span>{Math.min(100, Math.round((card.saldo / program.carimbos_necessarios) * 100))}%</span></div>
                <Progress value={Math.min(100, (card.saldo / program.carimbos_necessarios) * 100)} className="h-2 bg-black/25" />
              </div>
            )}
          </div>
          {card ? (
