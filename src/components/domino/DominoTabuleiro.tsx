...
export const DominoTabuleiro = ({ usuarioId, salaId, numeroSala, onVoltarAoLobby }: DominoTabuleiroProps) => {
  ...
  return (
    <div ref={containerRef} className="w-full h-screen bg-[#090610] text-white font-sans flex flex-col justify-between p-2 md:p-4 overflow-hidden select-none relative">
      {modalNotificacao.visivel && (
        <div className="absolute inset-0 bg-black/85 z-50 flex items-center justify-center p-4">
          <div className="max-w-sm bg-[#110D1A] border border-purple-950/60 p-8 rounded-2xl shadow-2xl text-center space-y-5">
            ...
            <p className="text-xs text-gray-300">{modalNotificacao.message}</p>
            <Button className="w-full bg-purple-600 text-white font-bold h-11" onClick={lidarComSaidaVoluntaria}>Voltar ao Lobby</Button>
          </div>
        </div>
      )}
      ...
      <div className="flex-grow my-1 bg-emerald-950 border-[4px] border-amber-950 rounded-[24px] relative flex flex-col items-center justify-center h-[53%] overflow-hidden">
        {mesaPedras.length === 0 ? (
          <div className="text-center font-bold text-xs py-6">
            {ambosJogadoresPresentes ? "Mesa de Dominó Limpa" : <span className="text-amber-500 animate-pulse">Aguardando oponente para iniciar...</span>}
          </div>
        ) : (
      ...
      <div className="bg-[#110D1A]/95 border border-purple-950/40 p-2.5 rounded-2xl h-[30%] flex flex-col justify-between">
        <div className="flex justify-center items-center gap-1.5 overflow-x-auto h-[75%] py-1">
          {minhasPedras.map((pedra, idx) => <PedraClassica key={idx} valor={pedra} onClick={() => tentarJogarPedra(pedra)} disabled={!meuTurno || !isPedraJogavel(pedra)} menor={true} destacada={meuTurno && isPedraJogavel(pedra)} />)}
        </div>
      </div>
    </div>
  );
